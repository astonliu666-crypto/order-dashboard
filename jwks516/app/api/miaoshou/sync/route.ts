...
export async function POST(request:Request){
  try{
    ...
    const account=String(input.account||"1");
    const appKey=account==="2"?process.env.MIAOSHOU_APP_KEY_2:process.env.MIAOSHOU_APP_KEY;
    const secret=account==="2"?process.env.MIAOSHOU_APP_SECRET_2:process.env.MIAOSHOU_APP_SECRET;
    if(!appKey||!secret)return NextResponse.json({ok:false,message:`妙手账号${account}尚未填写完整密钥`},{status:503});
    ...
    return NextResponse.json({ok:success,account,packages,count:packages.length,code:raw?.code,message:raw?.message||"同步完成",rawCount:containers.length});
